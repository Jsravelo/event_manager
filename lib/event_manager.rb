...
def clean_phone_number(phone_number)
  ...
  if cleaned_numbers.length < 10 || cleaned_numbers.length > 11
    ...
    format_phone_number(cleaned_numbers[1..])

  elsif cleaned_numbers.length == 10
    format_phone_number(cleaned_numbers)
  end
end
...
def format_phone_number(phone_number)
  ...
  "(#{area_code}) #{three_first}-#{last_four}"
end
...
def registration_time_frequency(contents)
  ...
  time_frequency = contents.each_with_object(Hash.new(0)) do |row, result|
    date = row[:regdate]
  ...
  times_with_most_frequency = time_frequency.select { |_time, frequency| frequency == most_repeated_time }
...
def registration_day_frequency(contents)
  ...
  day_frequency = contents.each_with_object(Hash.new(0)) do |row, result|
    date = row[:regdate]
  ...
  days_with_most_frequency = day_frequency.select { |_day, frequency| frequency == most_repeated_day }
